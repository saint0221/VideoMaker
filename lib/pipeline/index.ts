...
export async function runPostScript(projectId: string) {
  ...
  try {
    ...
    const promptsMd = await runImagePrompter(projectId, topic, sceneDesignMd, scriptMd, undefined, project.loraTriggerWord, project.imageModel);
...
export async function resumePipeline(projectId: string) {
  ...
  try {
    ...
    if (!prompts) {
      ...
      emit(projectId, { type: 'status', status: 'running:prompts' });
      prompts = await runImagePrompter(projectId, topic, scene!, script!, undefined, project.loraTriggerWord, project.imageModel);
      updateStatus(projectId, 'done:prompts');
      emit(projectId, { type: 'status', status: 'done:prompts' });
    }
