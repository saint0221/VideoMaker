...
async function uploadToFal(apiKey: string, imageBuffer: Buffer, filename: string): Promise<string> {
  // Step 1: Initiate upload to get a presigned PUT URL
  ...
  // Step 2: PUT the binary data to the presigned URL
...
export async function runVideoGenerator(projectId: string): Promise<void> {
  ...
  for (const imageFile of imageFiles) {
    ...
    const imageUrl = await uploadToFal(apiKey, imageBuffer, imageFile);
